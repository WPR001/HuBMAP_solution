...
def get_weighted_mask(masks, scores, inmodels):
...
def get_weighted_box(boxes, scores, inmodels):
...
def wbf_masks(masks, 
    ...
    masks = masks[scores > 0]
    boxes = boxes[scores > 0]
    models = models[scores > 0]
    ...
    for i in range(len(masks)):
        ...
        if index != -1:
            ...
            weighted_boxes[index], weighted_scores[index] = get_weighted_box(new_boxes[index], new_scores[index], inmodels[index])
        else:
    ...
    for nmasks, nscores, wbox, inms in zip(new_masks, new_scores, weighted_boxes, inmodels):
        mask, score, conf_list = get_weighted_mask(nmasks, nscores, inms)
